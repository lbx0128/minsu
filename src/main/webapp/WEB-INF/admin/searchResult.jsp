    ...
    <div class="content">
        ...
        <select id="select-search" class="form-control">
            ...
            <c:forEach items="${searchType}" var="sety">
                <c:choose>
                    <c:when test="${sety.key.equals(type)}">
                        <option value="${sety.key}"
                                selected="selected">${sety.value}</option>
                    </c:when>
                    <c:otherwise>
            ...
            <ul class="pager">
                <c:choose>
                    <c:when test="${pageIndex <= 1}">
                        <li class="disabled">
                            <a href="hotel/hotelinfos/search/${type}/${keyword}/
                    ${pageIndex-1}">上一页</a>
                        </li>
                    </c:when>
                    <c:otherwise>
        ...
        $(document).ready(function () {
            ...
            $("#button-jump-index").click(function () {
                ...
                if (index <= ${pageNumber} && index >= 1)
                    window.location.href = "hotel/hotelinfos/search/${type}/${keyword}/"+ index;
            });
